...
export const OperationsProvider = () => {
  ...
  const [policesIp, setPolicesIp] = useState<string>(''); 
  ...
  useEffect(() => {
    ...
    socket.on('police:ip', (data: string) => {
      console.log(data);
      setPolicesIp(data);
    });
        ...
        <Outlet />
        {
          !isSupporting && Object.keys(agentsPosition).length ? (
            Object.keys(agentsPosition).map((client: string) => {
              ...
              return (
                <Fragment key={client}>
                  <a href={`http://${policesIp}:81/stream`} target='_blank' rel="noreferrer">
                    <PositionMarker position={agentsPosition[client] as LatLngExpression} ip={policesIp} />
                  </a>
                </Fragment>
              );
            })
